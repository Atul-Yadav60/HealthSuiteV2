...
CREATE INDEX IF NOT EXISTS idx_condition_updates_user_id ON public.condition_updates(user_id);
CREATE INDEX IF NOT EXISTS idx_condition_updates_condition_name ON public.condition_updates(condition_name);
CREATE INDEX IF NOT EXISTS idx_condition_updates_created_at ON public.condition_updates(created_at DESC);
...
CREATE POLICY "Users can view their own condition updates" ON public.condition_updates
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own condition updates" ON public.condition_updates
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own condition updates" ON public.condition_updates
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own condition updates" ON public.condition_updates
    FOR DELETE USING (auth.uid() = user_id);
...
CREATE TRIGGER update_condition_updates_updated_at 
    BEFORE UPDATE ON public.condition_updates 
    FOR EACH ROW 
    EXECUTE FUNCTION update_updated_at_column();